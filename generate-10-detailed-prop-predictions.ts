...
// Detailed analysis content for each prop (from the detailed analysis document)
...
async function main() {
  console.log('\n🎯 GENERATING 10 DETAILED PLAYER PROP PREDICTIONS\n');
  ...
  for (const propInfo of selectedProps) {
    try {
      ...
      if (prediction && prediction.recommended_bet) {
        // Enhance reasoning with detailed analysis
        const detailedAnalysis = detailedAnalyses[propInfo.id] || '';
        const enhancedReasoning = detailedAnalysis 
          ? `${prediction.reasoning}\n\n${detailedAnalysis}`
          : prediction.reasoning;
        
        // Save to database with enhanced reasoning
        const { error: saveError } = await supabase
          ...
          .upsert({
            ...
            reasoning: enhancedReasoning, // Use enhanced reasoning with detailed analysis
        ...
        if (!saveError) {
          ...
          console.log(`   ✅ Saved: ${prediction.recommended_bet} ${propInfo.line} ${marketName}`);
